...
public class MentorPostController {
    ...
    @PostMapping("")
    public MentorPost post(@RequestBody MentorPostDTO tutorPostDTO) {
        return mentorPostService.savePost(tutorPostDTO);
    }

    /* 구인 게시판에 등록된 전체 글 조회 */
    @Operation(summary = "구인 게시판 전체 글 조회", description = "구인 게시판에 등록된 전체 글을 조회합니다.(카테고리 상관 없이 모든 글 조회)")
    @GetMapping("")
    public List<MentorPost> getAllPosts() {
    ...
    public List<MentorPost> getPostsByWriter(@PathVariable("username") String username) {
    ...
    @GetMapping("/{id}")
    public MentorPost getPostById(@PathVariable("id") Long id) {
    ...
    public List<MentorPost> getPostsByTitle(@PathVariable("title") String title) {
    ...
    public List<MentorPost> getPostsByCategory(@PathVariable("category") Category category) {
